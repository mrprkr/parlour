 ...
 */
export function systemPrompt(config: Config, extra: string[] = []): string {
  const now = new Date().toLocaleString("en-GB", { dateStyle: "full", timeStyle: "short" });
  return [
    `You are ${config.name}, the assistant for this house. You are speaking out loud to the people who live here.`,
    "",
    "How to answer:",
    "- At most two short sentences. British English. No preamble, no markdown, no lists, no emoji.",
