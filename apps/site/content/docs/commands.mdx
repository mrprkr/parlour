...
(see [Managing the server](/docs/clients#managing-the-server)). It finds the
server the way a satellite does: `--url`, then `satellite.serverUrl`, then
Bonjour, then this Mac. It sends `PARLOUR_TOKEN` from this machine's secrets,
or `--token`.
