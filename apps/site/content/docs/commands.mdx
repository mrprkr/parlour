...
parlour doctor [--json] [--network]              which of the moving parts is down
...
parlour models fetch [--llm auto] [--voice]|suggest|status
parlour laya setup|status                        on-device decisions, their MCP server and skill
