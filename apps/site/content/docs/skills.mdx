...
paragraph. A skill with files of its own can be a directory instead, with the
rule in `SKILL.md`:
...
in what order, and how short the spoken reply should be. `parlour doctor` counts the skills it found, names any
file it could not read, and warns when a body is long enough to slow every
answer that uses it.
...
The first adds a server on the network. The second adds one this machine
