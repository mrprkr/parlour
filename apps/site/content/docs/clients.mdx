...
shows the same code under On the network. The code carries the token, so show
it only to phones you mean to let in. If the phone cannot reach the Mac by its
name, give the address it can: `parlour pair --host 192.168.1.20`.
