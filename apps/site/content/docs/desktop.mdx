...
app and the terminal always see the same configuration.
...
A build from your own checkout is unsigned, which is fine on the machine
that made it: copy `Parlour.app` into `/Applications` and, the first time,
open it with a right click. The dmg on a release is signed with a Developer
ID certificate and notarised, so that one opens the ordinary way, on a
...
Notarising means signing with the hardened runtime, and the hardened runtime
takes the microphone away from an app that has not asked for it, so
`src-tauri/Entitlements.plist` asks. `Info.plist` next to it is the sentence
macOS shows when it puts the question to the person using it. The recording
happens in ffmpeg rather than in the app, but ffmpeg is a child of the app
and the system attributes the request to whoever started it, so both files
belong to the app.

The release workflow sets the Apple environment variables Tauri reads, and
Tauri does the rest: a keychain of its own for the certificate, the signature,
and then notarising `Parlour.app` and stapling the ticket into it. It signs
the dmg but does not notarise it, and a disk image without a ticket of its
own is refused before anybody reaches the app inside, so `scripts/notarise.sh`
does that half and then asks Gatekeeper what it makes of both. That script
runs from a checkout too, where it finds no credentials, says so and stops.
...
The path is kept for as long as it is executable, so the app will not
replace it with the global copy. The dev build and the installed
`Parlour.app` share the same settings file, so when you are done, put the
global path back, or delete the file and let the app find `parlour` again.
