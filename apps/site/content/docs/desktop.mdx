...
On a machine that is not set up yet, the app opens on a setup page instead of
the tabs. It remembers where it got to, so you can close it and come back.
**Run setup again** in Settings brings it back later. Four steps:

1. **Parlour.** Find the `parlour` command, or install it with one button.
2. **What is missing.** Whether Node, ffmpeg and the config are in place, and
   a button that runs `parlour init --porcelain --yes` for the rest (ffmpeg and
   whisper from Homebrew, the models, the config), showing its output as it
   goes.
3. **The things it cannot work out.** The Home Assistant address and token,
   the Anthropic key, the wake word, the microphone (and asking macOS for
   permission to use it), and whether to let the rest of the house in with an
   access token.
4. **Check it over.** The doctor's list, and a button to start listening.
...
src/
  ...
  panels/           one file per tab, plus the setup overlay
...
- Put its absolute path in **The parlour command** on the Settings tab (or on
  the first step of setup). This takes effect at once.
