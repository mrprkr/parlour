...
| --- | --- |
| Onboarding | Opens by itself on a machine that is not set up yet, and from **Run setup again** in Settings after that. Four steps: find or install `parlour`, an Install button that runs `parlour init --porcelain --yes` with live output, the questions, and the doctor. |
...
- Put its absolute path in **The parlour command** on the Settings tab (or on
  the first step of onboarding). This takes effect at once.
