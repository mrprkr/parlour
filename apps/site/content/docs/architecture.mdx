...
(built-in: `laya-mlx`) that judges escalate-versus-local before the
generative turn. In `shadow` mode it only logs calibrated scores beside
whatever the local model did; in `triage` mode a high `needs_cloud` /
`needs_web` score skips local and hands the transcript to the cloud, while
...
worker, so transcripts never leave the machine for triage.
