...
Optionally, `llm.decision` names a System One decision model (built-in:
`jev`) that judges escalate-versus-local before the generative turn. In
...
and hands the transcript to the cloud, while a clear house or timer intent
keeps the escalate tool off so the answer stays on-box. Default is
`provider: "none"`, so local-only houses are unchanged. Triage needs
`TYPESAFE_API_KEY` in `secrets.env` and sends the transcript to TypeSafe.
