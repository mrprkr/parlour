...
The house can run without a cloud model and without web search. A provider
that cannot start (nearly always a missing key) is left out, and the doctor
reports it. A name that leads nowhere is a config mistake and throws, so a
typo cannot quietly leave you with the wrong model.
