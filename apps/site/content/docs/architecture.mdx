...
Any port can also expose `doctor(): Promise<Check[]>`. `parlour doctor`
joins every configured part's checks together and adds three of core's own.
The config parses, the network is let in, and something keeps Parlour running.
...
when they are imported. You select each slot in config by name:
...
Its default export is accepted when its `kind` matches. That is the whole
extension mechanism, and [Writing a provider](/docs/providers) walks you through
it.

You don't choose `secrets` and `service` in config. Core picks them by
platform. It uses the Keychain when `security` is on the PATH and a file
otherwise. It uses launchd on macOS. Elsewhere it throws an error saying that
a systemd provider would be a welcome contribution.
...
- **listening.** Frames go to the endpointer. It gives up after 2.5 s of
...
the kitchen cannot pick up something asked in the study. Each session
forgets its history after four minutes of silence. The id is also the
client's lane in the queue and its own wake word detector. So it decides
both what a client can see and how much of the house it can hold up.
...
becomes a list of tasks. Each is written to stand on its own and marked as
something to do, something to look up, or small talk. A question triage is
...
whether it should stay local or go to the cloud. The built-in one is
`laya-mlx`, wired up in `core/decision.ts`. It needs Apple silicon and
...
- **`shadow` mode** only logs the scores beside whatever the local model did.
- **`triage` mode** acts on them. A high `needs_cloud` or `needs_web` score
  skips the local model and hands the task to the cloud. A clear house or
  timer intent keeps the escalation tool off, so the answer stays on your Mac.
...
timestamp. The desktop app reads these to draw its status instead of
...
| Model | llama.cpp with a Qwen2.5 GGUF, kept warm | It comes with Parlour, so the house answers straight after `init`, with no LM Studio to install and no server to remember to start. It speaks the OpenAI API, so LM Studio, Ollama or anything else can take its place. |
