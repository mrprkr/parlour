...
- **Through Assist** (`assist: true`). `ha_assist` passes a command, word for
  word, to [Assist](https://www.home-assistant.io/voice_control/), Home
  Assistant's own voice assistant, and returns what it said. See below.
...
Assist is Home Assistant's built-in conversation agent. What you set up for it
keeps working when Parlour is the one listening:
...
with a `message` in its data. Turn `assist` off to leave custom sentences to
Home Assistant's own satellites.
