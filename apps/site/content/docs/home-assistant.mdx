...
bottom. That token controls the whole house, so it stays in `secrets.env`
with mode 600 and never goes into git.

## Setting it up

`parlour init` has a part of its own for this, because there are four ways
for it to go wrong and all four look identical from the outside: the wrong
address, no token, a token for something else, and the MCP Server
integration never added. So each one is checked as it is answered:

1. **The address.** It tries the one in your config, then
   `homeassistant.local:8123`, `homeassistant:8123` and `localhost:8123`,
   and takes the first that answers the way a Home Assistant does: `401`
   from `/api/` without a token, which a router's own login page does not.
   Nothing found means typing it, and a wrong one is not accepted quietly.
2. **The token.** It prints the exact page to make one on
   (`<your url>/profile/security`), reads it without echoing it, and checks
   it against `/api/` before moving on. Three goes: a pasted token is easy
   to truncate.
3. **The MCP Server integration.** If `<url>/mcp_server/sse` does not
   answer, it says where to add it and offers to look again once you have.
4. **The mute entity**, if you want one.

Say no to the first question and the house is left out of the config
entirely, rather than left in it and failing every `parlour doctor` run for
something you do not have. Run `init` again whenever you change your mind:
every answer defaults to what is already set, and a token that still works
is kept rather than asked for twice. `parlour secrets set HA_TOKEN` takes
one on stdin if you would rather not run the whole thing.
...
- **Over REST** (`rest: true`). Two general tools for what the MCP tools
  cannot express: `ha_get_state` reads any entity, and `ha_call_service`
  calls any service. The prompt tells the model to prefer a more specific tool when one
  fits.

Parlour connects to the MCP endpoint at `<url>/mcp_server/sse` with the
token. If that connection fails, the failure is logged and the REST tools
still reach the house, more slowly.
...
3. Model `parlour`. The prompt and the temperature on that page are ignored,
   because Parlour brings its own persona and its own tools.
...
To follow a sleeping mode, define the boolean and an automation that
tracks it:
...
You can then override it by hand for one night from the dashboard. If Home
