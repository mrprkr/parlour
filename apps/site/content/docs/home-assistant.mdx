...
`parlour doctor` does not fail on something you do not have. Run `init` again whenever you change your mind. Every answer
...
Parlour, and it covers every client: satellites, phones and the server's own
microphone. Empty, the default, means no mute.
...
with an `agent.config.json` and a `.env` beside it. If you ran that,
`parlour init` looks for `agent.config.json` in the working directory. When
there is nothing at the new path yet, it offers to convert it for you:
...
For one release, `AGENT_TOKEN` is still read as a fallback for
`PARLOUR_TOKEN`, so upgrading does not lock every phone and satellite out of
the house. The model id reported to Home Assistant's OpenAI Conversation
integration is now `parlour`, so edit that one field by hand.
