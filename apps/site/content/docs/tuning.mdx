...
and uv. `parlour laya setup` (the Models step of the desktop setup runs it)
makes a pinned environment in `~/Library/Caches/parlour/laya`, downloads the
checkpoint, sets `llm.decision` to laya-mlx in shadow mode, adds a `laya` MCP
server that gives the model `choose`, `rate`, `check` and `decide`, and writes
a `deciding-with-laya` skill saying when they are worth using. From a checkout
of the repository, `pnpm exec nx run laya:setup` makes one in `packages/laya`
instead, and Parlour uses it without further config.
