...
and `pip install laya-mlx`. From a checkout of the repository,
`pnpm exec nx run laya:setup` makes a pinned environment in `packages/laya`
instead, and Parlour uses it without further config. The first load downloads
