...
const SUMMARIES: Record<keyof typeof COMMANDS, string> = {
  ...
  try: "try one stage on its own: mic, wake, stt, llm, cloud, tts, ask",
  doctor: "check every moving part and name the broken one",
  service: "install, uninstall, start, stop, restart, status, logs",
  remote: "manage a server from here: status, pipeline, service, doctor, logs, restart",
