...
export const command: Command = {
  ...
  async run({ paths, argv }) {
    ...
    const options: InitOptions = { yes: values.yes === true, deps: values["no-deps"] !== true, report };
    try {
...
async function init(paths: Paths, options: InitOptions): Promise<void> {
  ...
  if (!(await runSetup({ paths, config, deps: options.deps }, report))) {
  ...
  let anthropicKey = secrets.anthropicKey ?? "";
  let token = secrets.token ?? "";

  if (role === "satellite") {
    report.step("The server");
    report.ok("A satellite finds the server with Bonjour, so all it needs is the");
    report.ok("same access token the server was given. Leave the address blank");
    report.ok("unless this network does not carry multicast.");
    const serverUrl = yes
    ...
    if (config.llm.cloud.provider === "anthropic") {
      ...
      // switch is turned on only by a key typed this run, never by one found
      // in the environment or kept from last time, so a run that asked
      // nothing (no terminal counts as asking nothing) cannot turn the cloud
      // on behind someone's back.
      if (yes || !canAsk()) {
        report.ok(anthropicKey ? "keeping the existing key" : "no key, so the local model is on its own");
      } else {
        const answer = await keepOrAsk("Anthropic API key", anthropicKey, yes);
  ...
  if (role === "server") {
    ...
    const voice = yes
      ? config.tts.voice
      : await ask("Speaking voice (bf_emma, bf_isabella, bm_george, bm_lewis)", config.tts.voice);
  ...
  // Written even when they came from the environment: a LaunchAgent starts
  // with none, and the file is what it reads.
