 ...
 * what is left here is where the results are kept (the secret store) and how the
...
export function providerFor(store: ConnectorStore, connector: Connector): OAuthClientProvider {
  return new ConnectorAuthProvider(store, connector, "http://127.0.0.1:0/callback", (url) => {
    ...
    log.warn(
      `${connector.name} needs signing in again: parlour connectors add ${connector.name} ${connector.url}`,
    );
 ...
 * exchange the code. Resolves once tokens are in the secret store.
 */
export async function authorise(store: ConnectorStore, connector: Connector): Promise<void> {
  ...
  const server = createServer((request, response) => {
    ...
    response
      ...
      .end(
        `<!doctype html><meta charset="utf-8"><title>Parlour</title>` +
