...
/**
 * The clock, injectable so a test can wind it forward rather than wait. The
 * defaults are the real ones and nothing else needs to know the seam exists.
 */
...
/**
 * Timers stay in core rather than in Home Assistant. A kitchen timer that
 * stops working when the network does is not a kitchen timer.
 */
export class Timers {
  #next = 1;
  ...
  constructor(announce: (text: string) => void, options: TimersOptions = {}) {
    ...
    return [
      defineTool(
        ...
        async (args) => {
          ...
          this.#timers.set(id, { id, label, firesAt: this.#now() + seconds * 1000, handle });
      ...
      defineTool(
        ...
        async () => {
          ...
          return [...this.#timers.values()]
            .map((t) => `${t.id}: ${t.label}, ${describe(Math.round((t.firesAt - this.#now()) / 1000))} left`)
            .join("; ");
        },
      ),
      defineTool(
        ...
        async (args) => {
          if (args.all) {
            for (const timer of this.#timers.values()) this.#clearTimeout(timer.handle);
            this.#timers.clear();
