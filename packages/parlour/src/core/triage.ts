...
export type TriageMode = "shadow" | "triage";
...
export function triageQuestions(state: TriageState): Record<string, DecisionQuestion> {
...
export async function runTriage(model: DecisionModel, state: TriageState): Promise<TriageVerdict> {
...
export function planTriage(verdict: TriageVerdict, options: PlanTriageOptions): TriagePlan {
...
function parseIntent(choice: string): TriageIntent {
  if ((TRIAGE_INTENTS as readonly string[]).includes(choice)) return choice as TriageIntent;
  return "unclear";
}
